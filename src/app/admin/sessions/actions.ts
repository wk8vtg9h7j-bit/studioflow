...
// Fill a class to capacity in one click. Used to show a class as full so demand
// shifts to the other sessions on the schedule. The class stays 'scheduled' so it
// still appears on the booking page — it just renders as full (and offers the
// waitlist) instead of disappearing.
//
// bookings is unique on (session_id, customer_id), so one account can't hold
// several seats. Each held seat therefore gets its own placeholder customer,
// created the same way walk-ins are. Seats are booked with credits_spent = 0 so
// no ledger movement happens; cancelling the booking releases the seat.
export async function fillSessionAction(formData: FormData) {
  await requireRole("admin", "/admin/sessions");
  ...
  const remaining = info.capacity - (seatCount ?? 0);
  if (remaining <= 0) return;

  for (let i = 0; i < remaining; i += 1) {
    const { data: customer } = await supabase
      .from("customers")
      .insert({ name: "Reserved", status: "active", source: "walk-in" })
      .select("id")
      .single();
    if (!customer) break;
