-- ============================================================================
-- StudioFlow — 3-hour booking + cancellation policy (hard-enforced).
--
-- Two changes, both server-side so the policy can't be bypassed from the UI:
--
--  1. book_session: move the online self-booking cutoff from 90 minutes to
--     3 hours before class start. Recreated verbatim from 0011 with only the
--     interval + message changed.
--
...
-- ----------------------------------------------------------------------------
-- book_session — self-booking closes 3 hours before class start.
-- ----------------------------------------------------------------------------
...
begin
  ...
  if v_session.starts_at <= now() + interval '3 hours' then raise exception 'Online booking has closed (class starts within 3 hours)'; end if;
