-- ============================================================================
-- StudioFlow — close online self-booking 90 minutes before class start.
--
-- Customers self-book via the book_session RPC (it resolves the calling user's
-- customer record with ensure_customer()). The admin register / walk-in path
-- inserts into bookings directly (registerExistingAction / registerWalkInAction)
-- and does NOT call this function, so this cutoff applies to online self-booking
-- ONLY — staff can still add someone at the door for an imminent class.
--
-- This recreates book_session verbatim from 0010, adding a single guard: block
-- when the class starts within 90 minutes. The pre-existing "already started"
-- guard is kept as-is (it still gives a clearer message once a class has begun).
-- ============================================================================
